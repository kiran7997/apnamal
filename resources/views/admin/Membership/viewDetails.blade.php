...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            ...
            <li><a href="{{ URL::to('admin/donations')}}"><i class="fa fa-dashboard"></i>  Listing All The Membership</a></li>
    ...
    <section class="invoice" style="margin: 15px;">
        ...
        <div class="row">
            <div class="col-xs-12">
                <h2 class="page-header" style="padding-bottom: 25px; margin-top:0;">
                    ...
                    @endif
                    <small style="display: inline-block">Join Date: {{ date('m/d/Y', strtotime($data->created_at)) }}</small>
                </h2>
            </div>
            <!-- /.col -->
        </div>
        ...
        <div class="row invoice-info">
            <div class="col-sm-12 invoice-col">
                 <p><strong>Transaction Id</strong>: {{isset($payment_details->razorpay_payment_id) ? $payment_details->razorpay_payment_id : null}}</p>

                {{ trans('labels.CustomerInfo') }}:
                <address>
                    <strong>{{ucwords(isset($data->name) ? $data->name : null)}}</strong><br>
                    <strong>Email</strong>: {{isset($data->email) ? $data->email : null}}<br>
                    <strong>Phone No</strong>: {{isset($data->mobile) ? $data->mobile : null}}<br>
                    <strong>Alternate Phone No</strong>: {{isset($data->phone) ? $data->phone : null}}<br>
                    <strong>State</strong>: {{$state}}<br>
                    <strong>City</strong>: {{isset($data->city) ? $data->city : null}}<br>
                    <strong>ZIP / Postal Code</strong> : {{isset($data->pincode) ? $data->pincode : null}}<br>
                </address>
                
                <p><strong>Address1</strong>:  {{isset($data->address1) ? $data->address1 : null}}</p>
                <p><strong>Address2</strong>:  {{isset($data->address2) ? $data->address2 : null}}</p>
                   
             @if($data->member_type == 1) 
               @if($data->membership_plan == 1)
                <p><strong>Books For Prime Membership</strong>: 2 At-a-time Multiple order</p>
                @elseif($data->membership_plan == 2)
                <p><strong>Books For Prime Membership</strong>: 4 At-a-time</p>
                @elseif($data->membership_plan == 3)
                <p><strong>Books For Prime Membership</strong>: 6 At-a-time</p>
                @elseif($data->membership_plan == 4)
                <p><strong>Books For Prime Membership</strong>: 9 At-a-time</p>
                @elseif($data->membership_plan == 5)
                <p><strong>Books For Prime Membership</strong>: 12 At-a-time</p>
                @elseif($data->membership_plan == 6)
            ...
            <p><strong>Books</strong>: {{$books_name}}</p>
           @if($data->months == 1)
         ...
         @elseif($data->magazines == 2)
          <p><strong>Magazines</strong>: Semi-Annual</p>
        @elseif($data->magazines == 3)
           <p><strong>Magazines</strong>: Annual</p>
        @endif
          <p><strong>How did you find us</strong>: {{isset($data->how_to_find) ? $data->how_to_find : null}}</p>
         </div>
            <!-- /.col -->
        </div>
        <!-- /.row -->
