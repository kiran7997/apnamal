...
class News extends Model
{
  public function getAllNews($data){
		...
		$categories = DB::table('news_to_news_categories')
			...
			$categories->select('news.*','image_categories.path as image_path','news_description.*', 'news_to_news_categories.categories_id');
    ...
    if($data['is_feature']==1){
		...
		$categories->where('news_description.language_id','=',Session::get('language_id'))->orderBy($sortby, $order)->groupBy('news.news_id');
  ...
  public function newsdetail($request){
    $news = DB::table('news')
          ...
          ->select('news.*','news_description.*','image_categories.path as image')
