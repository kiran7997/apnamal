...
class OrdersController extends Controller
{
	...
	public function order_tracking(Request $request){
            ...
            $result = array();
            $result['commonContent'] = $this->index->commonContent();
            return view("web.order_tracking", $title);
	}

	//orderTracking
	public function orderTracking(Request $request){
            $orders_status_history = DB::table('orders')
					->LeftJoin('orders_status_history', 'orders.orders_id', '=', 'orders_status_history.orders_id')
					->LeftJoin('orders_status_description', 'orders_status_history.orders_status_id', '=', 'orders_status_description.orders_status_id')
					->select('orders_status_description.orders_status_name')
            ...
            if(!empty($orders_status_history)){
                $status = 'ORDER ID# '.$request->orders_id.' is '.$orders_status_history->orders_status_name;
    ...
    public function place_order_chk_pincode(Request $request) {
        $chk_pincode = DB::table('pincodes')->where('code',$request->code)->get();
    ...
    public function get_tax(Request $request) {
        $tax_rate = DB::table('tax_rates')->where('tax_zone_id',$request->id)->first();
        ...
        if(session('coupon')){
            ...
            foreach($session_coupon_data as $session_coupon){
                if($session_coupon->discount_type=='percent'){
                    $discount += ($request->total*$session_coupon->amount)/100;
        ...
        if(!empty($tax_rate)){
            $tax = ($request->total*$tax_rate->tax_rate)/100;
            $array = array(
                'tax' => number_format($tax,2),
                'tax_percent' => round($tax_rate->tax_rate),
                'total' => number_format($tax+$final_total+$request->shipping,2)
            );
            return $array;
        }else{
            $array = array(
                'tax' => 0,
                'tax_percent' => 0,
                'total' => number_format($final_total+$request->shipping,2)
            );
            return $array;
        }
    }
    ...
    public function checkout(Request $request) {
        ...
        if (count($result['cart']) == 0) {
            ...
            if (!empty(session('coupon')) and count(session('coupon')) > 0) {
                $session_coupon_data = session('coupon');
                $result['coupon'] = $session_coupon_data;
...
//                }
            ...
            $result['zones'] = $this->shipping->zones(99);
            //get tax
...
//            echo '<pre>';
//            print_r($result['shipping_methods'][0]['services'][0]['rate']);
//            die;
//            echo $result['shipping_methods'][0]['services'][0]['rate']; die;
            //price
            $price = 0;
            if (count($result['cart']) > 0) {
            ...
            if(auth()->guard('customer')->check()){
                $default_address = DB::table('orders')->where('customers_id',auth()->guard('customer')->user()->id)->orderby('orders_id','asc')->first();
                if(!empty($default_address)){
                    $pincodes = DB::table('pincodes')->pluck('code')->toArray();
                    if(in_array($default_address->billing_postcode,$pincodes)){
                        $submitBlocked = 1;
                    }
                }
            }
...
//            die;
            return view("web.checkout", ['title' => $title])
                    ...
                    ->with('submitBlocked', $submitBlocked);
        }
    }
	...
	public function place_order(Request $request){
	    try
	    {
    		...
    		$payment_status = $this->order->place_order1($request);
    		if($payment_status=='success'){
    			return redirect('/thank-you')->with('success', Lang::get("website.Payment has been processed successfully"));
    		}else {
	...
	public function returns(Request $request) {
            ...
            $title = array('pageTitle' => 'My Order Returns');
            $result = $this->order->orders($request);
            return view("web.returns", ['title' => $title])->with('result', $result);
    }
    //viewMyOrder
  ...
  //            echo '<pre>';
 //            print_r($result['orders'][0]->products);
//            die;
            if ($result['res'] = "view-order") {
                return view("web.view-order", $title)->with(['result' => $result]);
            }else{
		...
		{
		    $order_return = OrderReturns::query()->where('order_product_id','=', $request->orders_products_id)->first();
		    if(!$order_return)
		    {
		        $order_return = new OrderReturns();
		    }
		    $order_return->order_product_id = $request->orders_products_id;
		    $order_return->reason = $request->reasons;
		    $order_return->comments = $request->message;
		    $order_return->status = 1;
		    $order_return->save();
		    return redirect()->back()->with(['message','Return request raised.']);
		}
		
		
		public function cancelItem($id, $product_id)
		{
		    // $id is Order ID
		    // $product_id is Products ID in Order
    		...
    		if($order_product)
    		{
                ...
                DB::table('products')->where('products_id', '=', $product_id)->update([
		...
		public function openreturnForm($id, $product_id)
		{
			$result = array();
			...
			if($order_data){
                ...
                $order_products = DB::table('orders_products')
    				...
    				$order[$index]['products']= $products_array;
			}
		  return view('web.returnModal',compact('order','product_id','orders_products_id','id'));
		}
		
        public function vendor_viewOrder(Request $request)
        {
            return view('web.vendor-view-order');
        }
