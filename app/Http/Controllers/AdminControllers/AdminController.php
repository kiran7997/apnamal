...
class AdminController extends Controller
{
	...
	public function dashboard(Request $request)
	{
		...
		$total_product = DB::table('products')->where('vendor', Auth::user()->id)->count();

		$result['profit'] = number_format($profit, 2);
		$result['total_money'] = number_format($sold_cost, 2);
		$result['total_product'] = $total_product;
		$result['sold_cost_vendor'] = 0;
		$result['sold_cost_vendor_pending'] = 0;
		$result['sold_cost_vendor_completed'] = 0;
		$sold_cost_vendor = 0;
		$sold_cost_vendor_pending = 0;
		$sold_cost_vendor_completed = 0;
		if (Auth()->user()->user_type == 2) {
			//                    $vendor_orders = DB::table('orders')
			//                            ->LeftJoin('orders_products', 'orders.orders_id', '=', 'orders_products.orders_id')
			//                            ->LeftJoin('products', 'orders_products.products_id', '=', 'products.products_id')
			//                            ->LeftJoin('currencies', 'currencies.code', '=', 'orders.currency')
			//                            ->where('products.vendor',Auth()->user()->id)
			//                            //->where('customers_id','!=','')
			//                            //->select('orders.*','currencies.*')
			//                            ->orderBy('orders.date_purchased','DESC')
			//                            ->get();
			//                    foreach($vendor_orders as $vendor_order){
			//                        $sold_cost_vendor += $vendor_order->final_price;
			//                    }
			...
			//                    echo '<pre>';
			//                    print_r($vendor_pending_orders_ids);
			//                    die;
		...
		$result['sold_cost_vendor'] = $sold_cost_vendor;
		$result['sold_cost_vendor_pending'] = $sold_cost_vendor_pending;
		$result['sold_cost_vendor_completed'] = $sold_cost_vendor_completed;
		$compeleted_orders = 0;
		$pending_orders = 0;
		foreach ($orders as $orders_data) {
