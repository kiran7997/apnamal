...
class ProductController extends Controller
{
  ...
  public function pending(Request $request){
      ...
      $title = array('pageTitle' => "Pending Products");
      $subCategories = $this->category->getter($language_id);
      if(auth()->user()->id==1){
         $products = $this->products->where('products_status','=', 0)->paginate(100);
      }else{
         $products = $this->products->where('products.vendor', '=', auth()->user()->id)->where('products_status','=', 0)->paginate(100);
      }
      ...
      return view("admin.products.pending",$title)->with('results', $results)->with('categories_id',$categories_id)->with('product',$product);
    }
public function approveProduct($id){
        $product = DB::table('products')->where('products_id', '=', $id)->update(['products_status' => 1]);
    ...
    public function declineProduct($id){
        $product = DB::table('products')->where('products_id', '=', $id)->update(['products_status' => 2]);

        Session::flash('message', 'Product declined.'); 
        Session::flash('alert-class', 'alert-success'); 
        return redirect()->back();
    }
  ...
  public function display(Request $request){
      ...
      if(auth()->user()->id==1){
         $products = $this->products->where('products.products_status','=',1)->paginate(100);
      }else{
         $products = $this->products->where('products.vendor', '=', auth()->user()->id)->paginate(100);
      }
      $results['products'] = $products;
      $results['currency'] = $this->myVarsetting->getSetting();
      $results['units'] = $this->myVarsetting->getUnits();
